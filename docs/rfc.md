...
- **【头号问题】自动"挑瞬间"会同质化**（今日端到端实验暴露，见 `experiments/today_e2e_20260606/`）：DS-V4 给六个窗口挑出了几乎相同的瞬间——都是"做这块 eink 屏"。根因：(a) synthesize 的 system prompt 缺多样化/去重约束、且倾向挑最高频主题而非有张力的具体细节；(b) ai_sessions 无逐条时间戳导致当天内容涌进每个窗口、各窗口素材雷同。对比：昨天人工挑瞬间的八张各不相同。候选解法：一次性看全天为 8 窗口各挑**互不相同**的瞬间；显式排除"项目元讨论"类元主题；补 session 消息级时间戳；引入健康数据源补"微信沉默"窗口。
