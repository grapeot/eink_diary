...
- **低信息量窗口的 fallback（已实现，A 组日记调性）**：synthesize(moment) 让 DS-V4 自己判窗口够不够一个值得画的瞬间，不够则返回 `FALLBACK` 信号；pipeline 收到后采集【今天全天】素材、用 `collage` 模式重画——"今日鸭哥拼贴"（质地镜头的具象表达：一个黏土微缩场景里鸭哥分身做白天 3-4 件事，仍是鸭哥世界、小羊肖恩黏土风、温暖富细节）。**刻意不做冷数据图表**（那会背叛"反生产力日记"定位、退回被否决的仪表盘）。极端低料（连几件事都凑不齐）的更软兜底（天气隐喻/单物静物）暂未实现，待需要时再加。
