...
时序精度的硬事实：**数据源均已具备逐条时间戳，可按窗口精确过滤。** 微信有逐条真实时间戳（CreateTime）；AI sessions 的 OpenCode export 和 Claude Code JSONL 源均已补上逐条时间戳（commit 0adf37c / PR #6 / commit f410a23），collector 用 frontmatter date + turn HH:MM 组合出精确 datetime 做窗口过滤。
...
└── server/           # Pi 端 FastAPI display server
```
...
- **物件朝向仍未完全治好**：已把规则从绝对方向（"屏背朝观众"）改成相对关系（"屏正面朝鸭哥的脸"），但实测画面朝向仍有出错（用户 2026-06-06 反馈"方向还是不对，先这样"）。待后续再调 prompt（或考虑给参考图/更强约束）。
- **`eink-diary run` 偏慢**：一次 run 串 collect(~7s) + DS-V4 + gpt-image-2(2K) + 推送，整体可能上百秒。功能通（屏已成功刷新），但对 cron 而言要注意超时设置。collect 占了几秒（跨微信分片 DB 全扫）。
- **低信息量窗口的 fallback（已实现，A 组日记调性）**：synthesize(moment) 让 DS-V4 自己判窗口够不够一个值得画的瞬间，不够则返回 `FALLBACK` 信号；pipeline 收到后采集【今天全天】素材、用 `collage` 模式重画——"今日鸭哥拼贴"（质地镜头的具象表达：同一幅彩铅手绘场景里鸭哥分身做白天 3-4 件事，温暖富细节）。**刻意不做冷数据图表**（那会背叛"反生产力日记"定位、退回被否决的仪表盘）。极端低料（连几件事都凑不齐）的更软兜底（天气隐喻/单物静物）暂未实现，待需要时再加。
